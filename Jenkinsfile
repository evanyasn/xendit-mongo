  ...
  environment {
    ...
    stages {
      ...
      stage('push') {
        steps {
          retry(10) {
            ...
            sh "gcloud docker -- push gcr.io/xendit-trial/xendit-mongo:${tag_ver}"
          }
      ...
      stage('deploy') {
        steps {
          // apply changes
          sh "kubectl apply -f xendit-mongo-deployment.yaml"
          sh "kubectl apply -f xendit-mongo-service.yaml"
        }
      }
    }
  }
